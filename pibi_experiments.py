...
from src.models.pibi import PIBIModel
...
def main(seed):
    ...
    ## Experiment 1
    n_int_points = 200  # Number of integration points
    ...
    for bin in freq_bins:
        ...
        for idx, component in enumerate(['real', 'imag']):
            ...
            current_path = root_path.joinpath('pibi', 'exp1', str(seed), str(int(temporal_frequency)), component)
            current_path.mkdir(parents=True, exist_ok=True)

            # Initialize PIBI model
            model = PIBIModel(
                n_hidden_neurons = 64,
                n_hidden_layers = 2,
                n_integration_points=n_int_points,
            ...
            # Uncomment the following line to print model summary
    ...
    np.savetxt(root_path.joinpath('pibi', 'exp1', str(seed), 'nmse.csv'), np.array(nmse_freq))
    np.savetxt(root_path.joinpath('pibi', 'exp1', str(seed), 'ncc.csv'), np.array(ncc_freq))
    ...
    for M in n_mics:
        ...
        for idx, component in enumerate(['real', 'imag']):
            # Path to save results
            current_path = root_path.joinpath('pibi', 'exp2', str(seed), str(M), component)
            current_path.mkdir(parents=True, exist_ok=True)
    ...
    np.savetxt(root_path.joinpath('pibi', 'exp2', str(seed), 'nmse.csv'), np.array(nmse_mic))
    np.savetxt(root_path.joinpath('pibi', 'exp2', str(seed), 'ncc.csv'), np.array(ncc_mic))


    ## Experiment 3
    integration_points = [52,100,152,200,252,300,352,400,452,500,552,600] # Number of integration points (52, 152, 252... are chosen to be divisible by 4)
    bin = 25    # Frequency bin to evaluate the model
    temporal_frequency = freq_axis[bin]
    ...
    for N_int in integration_points:
        
        test_sound_field_estimated = torch.zeros((n_grid_points ** 2, 2))

        for idx, component in enumerate(['real', 'imag']):
            # Path to save results
            current_path = root_path.joinpath('pibi', 'exp3', str(seed), str(N_int), component)
            current_path.mkdir(parents=True, exist_ok=True)
        ...
        nmse_int.append(10 * torch.log10(torch.sum(nmse)).item())
        ncc_int.append(ncc.item())
        
        print(f'N_int: {N_int} - nmse: {nmse_int[-1]:.2f} - ncc: {ncc_int[-1]:.2f}')

    np.savetxt(root_path.joinpath('pibi', 'exp3', str(seed), 'nmse.csv'), np.array(nmse_int))
    np.savetxt(root_path.joinpath('pibi', 'exp3', str(seed), 'ncc.csv'), np.array(ncc_int))
